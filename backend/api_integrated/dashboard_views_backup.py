    ...
    try:
        ...
        response_data = {
            'weekly_calories': {
                ...
                'avg_daily_calories': int(sum(valid_calories) / len(valid_calories)) if valid_calories else 0,
                'max_day_calories': max(valid_calories) if valid_calories else 0,
                'chart_max': max(3000, max(valid_calories)) if valid_calories else 3000
            },
            'weight_data': {
...
def weight_records(request):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            return Response({
                'success': False,
                'error': f'잘못된 데이터입니다: {str(e)}'
            }, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            print(f"❌ 체중 기록 저장 실패: {e}")
            return Response({
                'success': False,
