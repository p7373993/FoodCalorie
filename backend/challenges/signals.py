...
def trigger_daily_challenge_judgment(sender, instance, created, **kwargs):
    ...
    try:
        ...
        active_challenges = UserChallenge.objects.filter(
            user=instance.user, status="active", remaining_duration_days__gt=0
        )
        ...
        for user_challenge in active_challenges:
            ...
            judgment_service.judge_daily_challenge(
                user_challenge, instance.date
            )

            logger.info(
                f"Daily challenge judgment triggered for user {instance.user.id} on {instance.date}"
            )
        ...
        for user_challenge in active_challenges:
            ...
            try:
                daily_record = DailyChallengeRecord.objects.get(
                    user_challenge=user_challenge, date=instance.date
                )
                ...
                logger.info(
                    f"Daily challenge re-judgment triggered for user {instance.user.id} on {instance.date}"
                )
