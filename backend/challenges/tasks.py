...
def daily_challenge_judgment_batch():
    ...
    active_challenges = UserChallenge.objects.filter(
        status="active", remaining_duration_days__gt=0
    )
    ...
    for user_challenge in active_challenges:
        try:
            ...
            existing_record = DailyChallengeRecord.objects.filter(
                user_challenge=user_challenge, date=yesterday
            ).exists()

            if not existing_record:
                # 기록이 없으면 판정 실행 (자동으로 실패 처리됨)
                judgment_service.judge_daily_challenge(
                    user_challenge, yesterday
                )
                processed_count += 1

        except Exception as e:
            logger.error(
                f"Error processing daily judgment for user {user_challenge.user.id}: {str(e)}"
            )
            failed_count += 1

    logger.info(
        f"Daily judgment batch completed: {processed_count} processed, {failed_count} failed"
    )
    return {
...
def weekly_cheat_reset_batch():
    ...
    try:
        ...
        logger.info(
            f"Weekly cheat reset completed: {updated_count} challenges updated"
        )
        return {
            "success": True,
...
def update_challenge_durations():
    ...
    try:
        ...
        logger.info(
            f"Challenge durations updated: {updated_count} updated, {completed_count} completed"
        )
        return {
            "success": True,
            "updated_count": updated_count,
    ...
    try:
        ...
        for challenge in old_challenges:
            ...
            deleted_count = DailyChallengeRecord.objects.filter(
                user_challenge=challenge, date__lt=cutoff_date
            ).delete()[0]

            deleted_records += deleted_count

        logger.info(
            f"Old records cleanup completed: {deleted_records} records deleted"
        )
        return {
            "success": True,
...
def generate_daily_statistics():
    ...
    try:
        ...
        # 전체 통계 계산
        total_active_challenges = UserChallenge.objects.filter(
            status="active"
        ).count()
        total_completed_challenges = UserChallenge.objects.filter(
            status="completed"
        ).count()
        ...
        success_rate = (
            (
                yesterday_records.filter(is_success=True).count()
                / yesterday_records.count()
                * 100
            )
        ...
        # 평균 연속 성공 일수
        avg_streak = (
            UserChallenge.objects.filter(status="active").aggregate(
