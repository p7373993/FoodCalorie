    ...
    def _generate_result_message(self, user_challenge, statistics, is_completed):
        ...
        if is_completed:
            ...
            if success_rate >= 80:
                ...
                return f"💪 아쉽지만 {success_rate}%로 챌린지를 마쳤습니다. 다음엔 더 좋은 결과를 만들어보세요!"
        else:
            remaining_days = user_challenge.remaining_duration_days
            current_streak = statistics['current_streak']
            if remaining_days > 0:
                return f"🔥 현재 {current_streak}일 연속 성공 중! 남은 {remaining_days}일도 화이팅!"
            else:
                return f"⏰ 챌린지 기간이 만료되었습니다. 최고 연속 기록: {statistics['max_streak']}일"
