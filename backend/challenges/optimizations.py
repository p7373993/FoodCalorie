...
class OptimizedChallengeQueries:
    ...
    def get_active_challenges_with_room_info() -> QuerySet:
        ...
        return (
            UserChallenge.objects.select_related("user", "room")
            .filter(status="active", remaining_duration_days__gt=0)
    ...
    def get_leaderboard_optimized(room_id: int, limit: int = 50) -> QuerySet:
        ...
        return (
            UserChallenge.objects.select_related("user")
            .filter(room_id=room_id, status="active")
            .order_by(
                "-current_streak_days",
                "-total_success_days",
    ...
    def get_user_challenge_with_records(user_id: int) -> QuerySet:
        ...
        from .models import DailyChallengeRecord, UserChallenge
    ...
    def get_user_badges_optimized(user_id: int) -> QuerySet:
        ...
        return (
            UserChallengeBadge.objects.select_related(
                "badge", "user_challenge__room"
            )
    ...
    def get_challenge_statistics_bulk(room_id: int) -> Dict[str, Any]:
        ...
        daily_stats = DailyChallengeRecord.objects.filter(
            user_challenge__room_id=room_id, date__gte=week_ago
        ).aggregate(
    ...
    def get_trending_challenges() -> QuerySet:
        ...
        return (
            ChallengeRoom.objects.annotate(
                active_participants=Count(
                    "participants", filter=Q(participants__status="active")
                ),
            ...
            .filter(is_active=True, active_participants__gt=0)
...
class BulkOperations:
    ...
    @staticmethod
    def bulk_update_streak_days(
        user_challenges: List, streak_updates: List[int]
    ):
        """연속 성공 일수 벌크 업데이트"""
        from .models import UserChallenge
    ...
    def bulk_create_daily_records(records_data: List[Dict]):
        ...
        records = [DailyChallengeRecord(**data) for data in records_data]

        DailyChallengeRecord.objects.bulk_create(
            records, ignore_conflicts=True  # 중복 방지
        )
    ...
    def bulk_reset_weekly_cheat_counts():
        ...
        return UserChallenge.objects.filter(status="active").update(
    ...
    def bulk_award_badges(user_badge_data: List[Dict]):
        ...
        badges = [UserChallengeBadge(**data) for data in user_badge_data]

        UserChallengeBadge.objects.bulk_create(
            badges, ignore_conflicts=True  # 중복 방지
        )
...
class PerformanceMonitor:
    ...
    def measure_query_time(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            logger.info(
                f"{func.__name__} - Time: {execution_time:.3f}s, Queries: {query_count}"
            )
    ...
    def get_database_stats():
        ...
        stats = {
            ...
            "avg_streak_days": UserChallenge.objects.filter(
                status="active"
            ).aggregate(avg=Avg("current_streak_days"))["avg"]
            or 0,
        }
...
class QueryOptimizationMiddleware:
    ...
    def __call__(self, request):
        ...
        if hasattr(connection, "queries") and len(connection.queries) > 20:
            ...
            duplicates = {
                sql: count for sql, count in query_counts.items() if count > 1
            }
            if duplicates:
