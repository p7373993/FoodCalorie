...
class ChallengeJudgmentService:
    ...
    def judge_daily_challenge(
        self, user_challenge: UserChallenge, target_date: date
    ):
        """
        ...
        with transaction.atomic():
            ...
            # 해당 날짜의 총 칼로리 계산
            total_calories = self._calculate_daily_calories(
                user_challenge.user, target_date
            )
            ...
            is_success = self._judge_success(
                user_challenge,
                target_date,
                total_calories,
                target_calories,
            ...
            # 일일 기록 생성/업데이트
            daily_record, created = (
                DailyChallengeRecord.objects.update_or_create(
                    user_challenge=user_challenge,
                    date=target_date,
                    defaults={
            ...
            # 연속 성공 일수 업데이트
            self._update_streak(
                user_challenge, target_date, is_success, is_cheat_day
            )
            ...
            logger.info(
                f"Daily judgment completed: User {user_challenge.user.id}, "
                f"Date {target_date}, Success: {is_success}"
            )
    ...
    def _calculate_daily_calories(self, user, target_date: date) -> float:
        """해당 날짜의 총 칼로리 계산"""
        total_calories = (
            MealLog.objects.filter(user=user, date=target_date).aggregate(
                total=Sum("calories")
            )["total"]
            or 0.0
        )
    ...
    def _update_streak(
        ...
        if is_success:
            if not is_cheat_day:
                ...
                # 최대 연속 성공 일수 업데이트
                if (
                    user_challenge.current_streak_days
                    > user_challenge.max_streak_days
                ):
                    user_challenge.max_streak_days = (
                        user_challenge.current_streak_days
                    )
    ...
    def _update_statistics(self, user_challenge: UserChallenge):
        ...
        user_challenge.total_success_days = records.filter(
            is_success=True
        ).count()
        user_challenge.total_failure_days = records.filter(
            is_success=False
        ).count()
        user_challenge.save()
...
class CheatDayService:
    ...
    def request_cheat_day(
        ...
        # 주간 치팅 사용 횟수 확인
        if (
            user_challenge.current_weekly_cheat_count
            >= user_challenge.user_weekly_cheat_limit
        ):
            return {
                "success": False,
        ...
        existing_request = CheatDayRequest.objects.filter(
            user_challenge=user_challenge, date=target_date
        ).first()
        ...
        return {
            "success": True,
            "message": "치팅이 승인되었습니다.",
            "remaining_cheats": user_challenge.user_weekly_cheat_limit
            - user_challenge.current_weekly_cheat_count,
        }
    ...
    def get_weekly_cheat_status(self, user_challenge: UserChallenge) -> dict:
        ...
        return {
            ...
            "remaining": user_challenge.user_weekly_cheat_limit
            - user_challenge.current_weekly_cheat_count,
        }
...
class ChallengeStatisticsService:
    ...
    def get_user_statistics(self, user_challenge: UserChallenge) -> dict:
        ...
        success_rate = (
            (success_days / total_days * 100) if total_days > 0 else 0
        )
        ...
        recent_success_rate = (
            (
                recent_records.filter(is_success=True).count()
        ...
        try:
            ...
            participants = (
                UserChallenge.objects.filter(
                    room_id=room_id, status__in=["active", "completed"]
                )
                .select_related("user", "room")
                .order_by(
                    "-current_streak_days",  # 현재 연속 성공일 우선
                    "-max_streak_days",  # 최대 연속 성공일
                    "-total_success_days",  # 총 성공일
                    "total_failure_days",  # 실패일 적은 순
                )[:limit]
            )
            ...
            for rank, participant in enumerate(participants, 1):
                # 성공률 계산
                total_days = (
                    participant.total_success_days
                    + participant.total_failure_days
                )
                success_rate = (
                    (participant.total_success_days / total_days * 100)
                    if total_days > 0
                    else 0
                )
                ...
                if participant.last_activity_date:
                    days_since_last_activity = (
                        timezone.now().date() - participant.last_activity_date
                    ).days
                ...
                leaderboard.append(
                    {
                        ...
                        "total_failure_days": participant.total_failure_days,
                        "success_rate": round(success_rate, 1),
                        "status": participant.status,
                        "days_since_last_activity": (days_since_last_activity),
                        "challenge_start_date": (
                            participant.challenge_start_date.isoformat()
                        ),
                        "remaining_days": participant.remaining_duration_days,
                    }
                )
            ...
            logger.error(
                f"Error generating leaderboard for room {room_id}: {str(e)}"
            )
            return []
...
class WeeklyResetService:
    ...
    def reset_weekly_cheat_counts(self):
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"Weekly cheat counts reset for {updated_count} "
                f"active challenges"
            )
