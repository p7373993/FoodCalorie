...
urlpatterns = [
    # 챌린지 방 목록 (인증 없이 접근 가능)
    path(
        "rooms/",
        views.ChallengeRoomListView.as_view(),
        name="challenge-room-list",
    ),
    # DRF Router URLs
    ...
    path(
        "my/extend/",
        views.ExtendChallengeView.as_view(),
    ...
    path(
        "cheat/status/", views.CheatStatusView.as_view(), name="cheat-status"
    ),
    # 순위 및 통계
    path(
        "leaderboard/<int:room_id>/",
