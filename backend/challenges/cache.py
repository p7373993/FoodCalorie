...
class ChallengeCache:
    ...
    @staticmethod
    def set(
        key_type: str, value: Any, timeout: Optional[int] = None, **kwargs
    ) -> bool:
        """캐시에 데이터 저장"""
        cache_key = ChallengeCache.get_cache_key(key_type, **kwargs)
        if timeout is None:
    ...
    def delete_pattern(pattern: str) -> int:
        ...
        try:
            # Redis 백엔드를 사용하는 경우
            if hasattr(cache, "_cache") and hasattr(
                cache._cache, "delete_pattern"
            ):
                deleted_count = cache._cache.delete_pattern(pattern)
                logger.debug(
                    f"Cache pattern deleted: {pattern} ({deleted_count} keys)"
                )
                return deleted_count
            else:
                # 다른 캐시 백엔드의 경우 개별 삭제
                logger.warning(
                    "Pattern deletion not supported for current cache backend"
                )
                return 0
        except Exception as e:
            logger.error(
                f"Cache pattern delete error for pattern {pattern}: {str(e)}"
            )
            return 0
    ...
    def warm_up_cache():
        ...
        from .models import ChallengeBadge, ChallengeRoom
        from .services import ChallengeStatisticsService

        try:
            # 활성 챌린지 방 목록 캐시
            active_rooms = list(
                ChallengeRoom.objects.filter(is_active=True).values()
            )
            ChallengeCache.set("room_list", active_rooms)

            # 활성 배지 목록 캐시
            active_badges = list(
                ChallengeBadge.objects.filter(is_active=True).values()
            )
...
class CachedLeaderboardService:
    ...
    @staticmethod
    def get_leaderboard(
        room_id: int, limit: int = 50, force_refresh: bool = False
    ) -> List[Dict]:
        """캐시된 리더보드 조회"""
        ...
        # 캐시 미스 또는 강제 새로고침 - 직접 데이터베이스 쿼리
        leaderboard = CachedLeaderboardService._generate_leaderboard(
            room_id, limit=100
        )
    ...
    @staticmethod
    def _generate_leaderboard(room_id: int, limit: int = 50) -> List[Dict]:
        """리더보드 데이터 직접 생성"""
...
class CachedUserStatsService:
    ...
    def get_user_statistics(
        user_challenge, force_refresh: bool = False
    ) -> Dict:
        """캐시된 사용자 통계 조회"""
...
class CachedRoomService:
    ...
    @staticmethod
    def get_room_info(
        room_id: int, force_refresh: bool = False
    ) -> Optional[Dict]:
        """캐시된 챌린지 방 정보 조회"""
        cache_key_params = {"room_id": room_id}
            ...
            for cache_type in cache_types:
                if cache_type == "user_cache":
                    user_id = kwargs.get("user_id") or (
                        args[0].user.id if hasattr(args[0], "user") else None
                    )
                    if user_id:
                    ...
                    room_id = kwargs.get("room_id") or (
                        args[0].room.id if hasattr(args[0], "room") else None
                    )
                    if room_id:
