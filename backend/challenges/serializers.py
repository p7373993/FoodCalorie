...
class UserChallengeSerializer(serializers.ModelSerializer):
    ...
    room_target_calorie = serializers.IntegerField(
        source="room.target_calorie", read_only=True
    )
    ...
    def validate(self, data):
        ...
        try:
            room = ChallengeRoom.objects.get(
                id=data["room_id"], is_active=True
            )
            data["room"] = room
        ...
        if data["user_height"] < 100 or data["user_height"] > 250:
            raise serializers.ValidationError(
                "키는 100cm~250cm 사이여야 합니다."
            )

        if data["user_weight"] < 30 or data["user_weight"] > 300:
            raise serializers.ValidationError(
                "몸무게는 30kg~300kg 사이여야 합니다."
            )

        if data["user_target_weight"] < 30 or data["user_target_weight"] > 300:
            raise serializers.ValidationError(
                "목표 체중은 30kg~300kg 사이여야 합니다."
            )

        # 챌린지 기간 검증
        if (
            data["user_challenge_duration_days"] < 7
            or data["user_challenge_duration_days"] > 365
        ):
            raise serializers.ValidationError(
                "챌린지 기간은 7일~365일 사이여야 합니다."
            )
...
class DailyChallengeRecordSerializer(serializers.ModelSerializer):
    ...
    user_challenge_id = serializers.IntegerField(
        source="user_challenge.id", read_only=True
    )
    username = serializers.CharField(
        source="user_challenge.user.username", read_only=True
    )
