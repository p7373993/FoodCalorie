...
class ChallengeRoom(models.Model):
    ...
    # 더미 데이터 관리
    dummy_users_count = models.IntegerField(
...
class UserChallenge(models.Model):
    ...
    CHEAT_LIMIT_CHOICES = [(0, "0회"), (1, "1회"), (2, "2회"), (3, "3회")]

    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="user_challenges"
    )
    room = models.ForeignKey(
        ChallengeRoom, on_delete=models.CASCADE, related_name="participants"
    )
    ...
    user_target_weight = models.FloatField(
        validators=[MinValueValidator(30), MaxValueValidator(300)]
    )  # kg
    user_challenge_duration_days = models.IntegerField(
        validators=[MinValueValidator(7), MaxValueValidator(365)]
    )
    user_weekly_cheat_limit = models.IntegerField(
        choices=CHEAT_LIMIT_CHOICES, default=1
    )
    ...
    max_streak_days = models.IntegerField(
        default=0, validators=[MinValueValidator(0)]
    )
    remaining_duration_days = models.IntegerField(
        validators=[MinValueValidator(0)]
    )
    current_weekly_cheat_count = models.IntegerField(
        default=0, validators=[MinValueValidator(0)]
    )

    # 통계
    total_success_days = models.IntegerField(
        default=0, validators=[MinValueValidator(0)]
    )
    total_failure_days = models.IntegerField(
        default=0, validators=[MinValueValidator(0)]
    )
    ...
    def challenge_end_date(self):
        """챌린지 종료 예정일"""
        return self.challenge_start_date + timezone.timedelta(
            days=self.user_challenge_duration_days
        )
...
class DailyChallengeRecord(models.Model):
    ...
    user_challenge = models.ForeignKey(
        UserChallenge, on_delete=models.CASCADE, related_name="daily_records"
    )
    ...
    is_cheat_day = models.BooleanField(
    ...
    def __str__(self):
        status = "성공" if self.is_success else "실패"
        return f"{self.user_challenge.user.username} - {self.date} ({status})"
...
class CheatDayRequest(models.Model):
    ...
    user_challenge = models.ForeignKey(
        UserChallenge, on_delete=models.CASCADE, related_name="cheat_requests"
    )
    date = models.DateField()
...
class ChallengeBadge(models.Model):
    ...
    condition_type = models.CharField(
        max_length=50, choices=CONDITION_TYPE_CHOICES
    )
...
class UserChallengeBadge(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="challenge_badges"
    )
    ...
    # 배지 획득 시점의 챌린지 정보
    user_challenge = models.ForeignKey(
        UserChallenge, on_delete=models.CASCADE, null=True, blank=True
    )
